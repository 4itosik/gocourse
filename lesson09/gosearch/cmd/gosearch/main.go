...
var (
	docsCountTotal = promauto.NewCounter(prometheus.CounterOpts{
		Name: "docs_count_total",
		Help: "Количество документов в хранилище.",
	})
	indexCountTotal = promauto.NewCounter(prometheus.CounterOpts{
		Name: "index_count_total",
...
func (gs *gosearch) init() {
	...
	indexCount := gs.index.Count()
	indexCountTotal.Add(float64(indexCount))
	docsCount := gs.storage.Count()
	docsCountTotal.Add(float64(docsCount))
}
